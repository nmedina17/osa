...
knitr::opts_chunk$set(echo = F, warning = F, message = F); here::i_am("article/methods.Rmd")
```
...
Edge effects were studied by dividing the secondary forest area into six 50 m strata spanning 0 - 300 m away from the primary forest edge on the S and NW sides, going inward to the E (Fig \@ref(fig:map)), using available GIS software (*ArcGIS 10 from 2010*, [esri.com](esri.com), and *QGIS 2 from 2013*, [qgis.org](qgis.org)).
...
The equation used was: $$ {AGB} = 0.0673 (ρ D^2 H)^{0.976} $$ with diameter *D* in cm, height *H* in m, and wood density or specific gravity *p* in *g cm^-3^*.
...
For all response variables, data were analyzed using plot medians and subject to linear regression with distance to forest edge as the only independent variable, after checks for residual normality and equal variance assumptions, using `r version$version.string`.
...
Data and code are available at [nmedina17.github.com/osa](nmedina17.github.com/osa).

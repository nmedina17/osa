...
**See () for an ecosystem description.**
...
This focal secondary forest area was roughly triangular, surrounded by primary forest on the two S and NW sides *(Fig 1)* and a wide service road on the third NE border, with primary forest beyond it.
...
Edge effects were studied by dividing the secondary forest area into six 50 m strata spanning 0 - 300 m away from the primary forest edge on the S and NW sides, going inward to the E *(Fig 1)*, using GIS software *(ArcGIS, QGIS)*.
...
The total area of the 30 census plots equaled ~1 ha or 5% of the total secondary forest stand area, which is comparable to similar studies *(Onyekwelu & Olabiwannu 2016)*.
...
Tree height was recorded by measuring distances to both the crown and the stem at chest height (~2.7 m) using a rangefinder *(Bushnell, Forestry Suppliers, Inc.)* and trangulating the missing side length.
...
Aboveground biomass (AGB) was estimated per stem using the allometric equation by *Chave et al. (2014)*.

The equation specifically for wet tropical forests was used, which has performed slightly better compared to both pantropical and previous other models by *Chave et al. (2005) (Ngomanda et al 2014, Alvarez et al 2012)*.
...
Original values from 2013 were later updated to modern values using the pipeline developed in the *BIOMASS* ( `r `packageVersion("BIOMASS")` ) R software package by *Réjou-Méchain et al. (2017)*, which notably replaced unknown wood density values with plot-level averages instead during stem biomass calculations. 
...
For all response variables, data were analyzed using plot medians and subject to linear regression with the distance to forest edge variable after checks for residual normality and equal variance assumptions using R `getRversion()` *(R Core Team 2019)*.
...
Non-linear regressions were run using the function *poly()* in the *stats* `packageVersion("stats")` R package *()*, 
...
Community analysis and ordination was done as a PERMANOVA with the *vegan* `packageVersion("vegan")` R package *()*.

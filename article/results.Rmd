...
library(here); i_am("article/results.Rmd"); source(here("figs/fig2.R"))
```
...
which explained 
`r "R2x100"`% of variance among distance strata medians 
*(Fig \@ref(fig:taxa)a)* -- 
...
and had `r "R2x100"`% variance explained among distance strata 
*(Fig \@ref(fig:taxa)b)*.
...
```{r fig3data}
source(here("figs/fig3.R")); succession <- taxaResultsTbl12 #pReplaced
```
...
`r "R2x100"`% variance among distance strata medians 

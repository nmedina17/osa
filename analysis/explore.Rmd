...
plotVarsStatTbl <- plotVarsTbl %>%
  ...
  mutate(normalTest = map(.x = statTest, #needstatcol1st
         ...
         statReport = map(.x = statTest, #formatsumlast
                        .f = ~ .x %>% summary())) %>%
  mutate(isNormal = map(.x = normalTest,
                           .f = ~ if_else(condition = .x$p.value > 0.05,
                                          true = T, false = F)),
         isHomosced = map(.x = varianceTest,
                          .f = ~ if_else(condition = .x$p > 0.05,
                                       true = T, false = F))) %>%
  mutate(isSignif = map_if(.x = statReport,
                           .p = ~ isNormal & isHomosced, #bug-notlogical
